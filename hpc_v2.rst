...
The arguments above specify the characteristics of a single job or a single compute node, rather than the total amount of cores or memory that you want for your computation as a whole. Moreover, It hasn’t actually launched any jobs yet. The cluster generates an standard LSF job script and submits that an appropriate number of times to the job queue. You can see the job script it will generate with,
...
This last case has 5 jobs running (on 5 different nodes). Each job has a total of 40 workers (8 processes with 5 threads). The total number of cores is 200, and 80GB of total (16GB x 5 jobs) memory, with 2GB memory limite per worker. 
